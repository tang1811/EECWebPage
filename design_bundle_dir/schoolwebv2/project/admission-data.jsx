// ═══════════════════════════════════════════════════════════
// admission-data.jsx — constants, helpers, reusable form atoms
// Exposes everything to window for cross-babel-scope sharing.
// Relies on Icon (from chrome.jsx, loaded earlier).
// ═══════════════════════════════════════════════════════════
...
const ADM_MONTHS = ['มกราคม','กุมภาพันธ์','มีนาคม','เมษายน','พฤษภาคม','มิถุนายน','กรกฎาคม','สิงหาคม','กันยายน','ตุลาคม','พฤศจิกายน','ธันวาคม'];

const ADM_KNEW = ['Facebook / Social', 'เว็บไซต์วิทยาลัย', 'เพื่อน / รุ่นพี่', 'ครูแนะแนว', 'ป้าย / สื่อโฆษณา', 'งานเปิดบ้าน Open House', 'อื่น ๆ'];

const ADM_RELATION = ['บิดา', 'มารดา', 'ผู้ปกครอง', 'ญาติ', 'อื่น ๆ'];
...
function TextField({ label, value, onChange, error, required, full, hint, ...rest }) {
  return (
...
function SelectField({ label, value, onChange, options, error, required, full, placeholder }) {
  return (
...
// ── Captcha ────────────────────────────────────────────────
function Captcha({ code, onRefresh, value, onChange, error }) {
...
// ── Choice card ────────────────────────────────────────────
function ChoiceCard({ title, desc, on, onClick }) {
  return (
...
function DocRow({ doc, file, onPick, onRemove }) {
...
// ── Summary group ──────────────────────────────────────────
function SummaryGroup({ title, onEdit, rows }) {
  return (
    <div className="sum">
...
Object.assign(window, {
  ADM_LEVELS, ADM_MAJORS, ADM_ROUNDS, ADM_MONTHS, ADM_KNEW, ADM_RELATION, ADM_DOCS,
  ADM_STATUS_STEPS, AdmPdpaText,
  admGenCaptcha, admFormatID, admFormatPhone, admGenAppNo,
